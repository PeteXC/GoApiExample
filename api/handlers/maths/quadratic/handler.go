...
var MathsAddGetMatcher = pathvars.NewExtractor("*/maths/quadratic")
...
func (h Handler) Handle(r *http.Request, w http.ResponseWriter) (err error) {
	...
	x := calculateQuadratic(int(req.NumberA), int(req.NumberB))
	fmt.Println(x)
...
func calculateQuadratic(a, b int) (x int) {
	x = a - b
	return
}
...
func NewHandler(log *zapray.Logger, offset string) (h Handler, err error) {
	return Handler{
		Log:    log,
		Offset: offset,
	}, err
